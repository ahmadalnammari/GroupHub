...
(function () {
    ...
    var groups = [{"token": 1, "name": "test"}, {"token": 2, "name": "test"}, {"token": 3, "name": "test"}];
    ...
    var employees = [
        ...
        findGroupByToken = function (token) {
            var group = null,
                l = groups.length,
                i;
            for (i = 0; i < l; i = i + 1) {
                if (groups[i].token === parseInt(token)) {
                    group = groups[i];
                    break;
                }
            }
    ...
    angular.module('myApp.memoryServices', [])
        ...
        .factory('Group', [
            function () {
                return {
                    getMyGroups: function (userToken) {
                        return groups;
                    },
                    getGroupDetails: function (groupToken) {
                        console.log(groupToken);
                        return findGroupByToken(groupToken);
                    }
                }
